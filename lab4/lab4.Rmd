...
```{r}
  ...
  n = 537
  x_i = c(0, 1, 2, 3, 4, 7)
  nu = c(229, 211, 93, 35, 7, 1)
 
  r = length(nu)
  lambda = sum(x_i*nu)/n

  P_n_nu = sum((nu - n*ppois(x_i, lambda=lambda))^2 / (n*ppois(x_i, lambda=lambda)))
...
```{r}
  ...
  P_n_nu = sum((nu - n*pnorm(x_i, mean=mu, sd=sd))^2 / (n*pnorm(x_i, mean=mu, sd=sd)))
...
```{r}
  ...
  arr1 = rnorm(100, mean=0, sd=2)
  arr2 = rnorm(150, mean=-1, sd=4)
  
  minn = min(arr1, arr2)
  maxx = max(arr1, arr2)
  
  X = seq.int(minn, maxx, abs(maxx + minn))
