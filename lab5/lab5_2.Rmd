...
```{r}
  F <- function(x, p1, p2) { 
    # distribution mix 
    ans = seq(from=0, to=0, length=length(x))
    
    for (i in 1:length(x)) {
      if (x[i] >= -3 && x[i] <= 3) {
        ans[i] = (p1 * pnorm(x[i], mean=0, sd=4) + p2 * (0.5 + (1/pi) * asin(x[i]/3)) + (1 - p1 - p2) * punif(x[i], 0, 5))
      }
      if (x[i] < -3) {
        ans[i] = (p1 * pnorm(x[i], mean=0, sd=4) + (1 - p1 - p2) * punif(x[i], 0, 5))
      }
      if (x[i] > 3) {
        ans[i] = (p1 * pnorm(x[i], mean=0, sd=4) + p2*1 + (1 - p1 - p2) * punif(x[i], 0, 5))
      }
    }
...
```{r}
  ...
  plot(x, y, main = "Distribution F, F_n", type="l", col="dark blue", lwd=2)
...
```{r}
  ...
  #for (i in 2:(r - 1)){
  #  nu[i] <- sum((mixed > delta[i]) & (mixed <= delta[i + 1]))
  #}
 
  #nu[1] = sum(as.numeric((mixed <= delta[1])))
  #nu[r] = sum(as.numeric((mixed > delta[r])))
  # -----calculate vector of freq-----
